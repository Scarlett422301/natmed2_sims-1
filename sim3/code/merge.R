...
for(set_t0 in unique(parameter_grid$set_t0)){
  for(covid_rate in unique(parameter_grid$covid_rate)){
    # get truth of cumulative incidence parameters
    # ey00, ey11, ey10
    truth_est = get_ows_truth1_survival(t0 = set_t0, covid_rate = covid_rate)[c(2,1,3)]
    
    # get truth of estimated effects
    truth_eff = get_ows_truth2_survival(t0 = set_t0, covid_rate = covid_rate)
    ...
    for(version in unique(parameter_grid$version)){
      ...
      for(X in 1:1000){
        print(X)
        setwd(paste0("/projects/dbenkes/jialu/vaccine/JnJ/results/covid_rate=", covid_rate))
      ...
      save(out, file = paste0("/projects/dbenkes/jialu/vaccine/JnJ/final_results/covid_rate", covid_rate, "_t", set_t0, "_version_", version, ".RData"))
    }
  }
}
